...
export default defineMcp({
  ...
  version: "0.1.0",
  instructions:
    "Tools for AI Income Systems Lab — read the signed-in member's profile and wins, post new wins to the Wall of Wins, and search published blog posts, newsletter issues, and pillar guides. All tools act as the authenticated user, respecting their tier and role.",
  auth: auth.oauth.issuer({
    issuer: `https://${projectRef}.supabase.co/auth/v1`,
    acceptedAudiences: "authenticated",
  }),
  tools: [getMyProfile, listMyWins, createWin, listRecentWins, searchContent],
});
