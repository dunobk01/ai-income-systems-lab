...
import { ArrowLeft, ExternalLink, Wrench, CheckCircle2, Users, Sparkles, Layers, Lightbulb } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { useAuth } from "@/lib/auth-context";
import { LockedView } from "./builders/product";
...
function ToolDetailPage() {
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      ...
      <header className="mt-6 glass rounded-2xl p-6 lg:p-8">
        <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-muted-foreground">
          <Wrench className="h-3 w-3" /> {tool.category}
        </div>
        <h1 className="text-3xl lg:text-4xl font-bold mt-3">{tool.name}</h1>
